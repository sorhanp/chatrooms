...
function handleRoomJoining(socket) {
    socket.on('join', function(room) {
        if(room.newRoom == currentRoom[socket.id]){
            socket.emit('joinResult', { //Print error if name is already in use
                success: false,
            ...
            socket.broadcast.to(oldRoom).emit ('message', { //Inform others that user has left the room and joined another
                text: nickNames[socket.id] + ' has left room "' + oldRoom + '" and joined room "' + room.newRoom + '".'
            });
        }
    });
}
...
function handleClientDisconnection(socket) {
    socket.on('disconnect', function() {
        socket.broadcast.to(currentRoom[socket.id]).emit ('message', { //Inform others that user has left the room and joined another
            text: nickNames[socket.id] + ' has left Chatrooms.'
        });
